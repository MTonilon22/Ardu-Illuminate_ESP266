...
#define RELAY_PIN D1 //defining the OUTPUT pin for BULB (D1)
    ...
    switch(type) {
        ...
        case WStype_TEXT:
            ...
            if(cmd == "poweron"){    
                ...
                if(dutyCycle > 0) {
                    digitalWrite(RELAY_PIN, RELAY_ON); // turn on the relay if duty cycle is greater than 0
                    delay(dutyCycle * 10); // wait for a time proportional to the duty cycle
                    digitalWrite(RELAY_PIN, !RELAY_ON); // turn off the relay
                    delay((100 - dutyCycle) * 10); // wait for the remaining time
                } else {
                    digitalWrite(RELAY_PIN, !RELAY_ON); // turn off the relay if duty cycle is 0
                }
              ...
              newPassword = cmd.substring(8);
               ...
               WiFi.begin(newSsid, newPassword); // Connect to new network
            }
